...
mpox_df <- full_join(
  ...
  select(country, iso2, iso3, date, page_title, est_pageviews, pageviews_ceil, pct_est_pageviews, cases, cases_moving_avg) |> 
  complete(fill = list(est_pageviews = 0, pct_est_pageviews = 0, cases = 0, cases_moving_avg = 0))  # fill in missing zero
...
# mpox_df |>
#   ggplot(aes(x = date, y = pct_est_pageviews, fill = country)) +
#   geom_col() +
#   scale_x_date(
