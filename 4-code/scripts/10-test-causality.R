...
test_df <- test_df |>
  mutate(
    pageviews_diff1 = c(NA, diff(pct_est_pageviews, lag = 1, differences = 1)),
    cases_diff1 = c(NA, diff(cases_moving_avg, lag = 1, differences = 1))
  )
...
test_df |>
  ggplot(aes(x = date, y = cases_diff1)) +
  geom_line() +
  labs(
...
mpox_ts <- test_df |>
  filter(!is.na(pageviews_diff1), !is.na(cases_diff1)) |>
  select(pageviews_diff1, cases_diff1) |>
  ts()

# determine optimal lag length for VAR model based on information criteria (e.g., AIC)
lag_selection <- VARselect(mpox_ts, lag.max = 36, type = "both")
